...
def collect_files(root: str | Path) -> list[Path]:
    """인덱싱 대상 파일 목록. SKIP_DIRS 아래는 통째로 제외."""
...
def chunk_code(text: str, rel_path: str) -> list[dict]:
    ...
    while i < n:
        ...
        size = 0
        j = i
        while j < n and size < CFG.chunk_size:
            buf.append(lines[j])
            size += len(lines[j]) + 1
            j += 1
        ...
        back = max(1, CFG.chunk_overlap // avg)
        i = max(i + 1, j - back)

    return chunks


def chunk_doc(text: str, rel_path: str) -> list[dict]:
