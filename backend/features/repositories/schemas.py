...
class BranchBindingCreateRequest(BaseModel):
    ...
    model_project_id: str = Field(min_length=1)
    active: bool = True

    @field_validator("frontend_project_id", "model_project_id")
    @classmethod
    def strip_non_blank_text(cls, value: str) -> str:
        normalized = value.strip()
        if not normalized:
...
class BranchBindingUpdateRequest(BaseModel):
    ...
    model_project_id: str | None = Field(default=None, min_length=1)
    active: bool | None = None

    @field_validator("model_project_id")
    @classmethod
