...
def test_branch_binding_contract_uses_full_ref_and_exact_ids() -> None:
    ...
    assert response.branch_ref == "refs/heads/backend_P"
    assert response.model_project_id == "vision--rag-v2"
...
def test_snapshot_list_keeps_repository_branch_and_reason() -> None:
    ...
    assert snapshot.branch_ref == "refs/heads/backend_P"
    assert snapshot.state is SnapshotState.ACCEPTED
    assert snapshot.upstream_reason == "INDEX_UPDATE_ACCEPTED"
